...
public class ScreenPatch
{
    ...
    public static void SceneTitles_Start(Scene_Titles __instance)
    {
        ...
        if (MetaFile.Data.FirstLaunch)
        {
            ...
            GameScreens.DCBAOOEJKDN.SetActive(false);
            Object.Destroy(GameObject.Find("Logo"));
            GameObject obj = Object.Instantiate(GameSprites.LHJLHHFLELN[1]);
            ...
            rt.sizeDelta *= 5;
            obj.transform.SetParent(GameScreens.NMLFLIDGDAF.transform, false);
        ...
        try
        {
            if (GameScreens.OBNLIIMODBI == 1001)
            {
                ...
                else if (Input.GetMouseButtonUp(0))
                {
                    ...
                    if (_menu != null)
                    {
                        GameSprites.JAAOOCGLBNN(_menu.KNOKHLBGDKO, 0.8f, 0.8f, 0.8f);
                    }
                }
                ...
                if (GameScreens.PDMDFGNJCPN >= Prefixes.Count + 1)
                {
                    ...
                    {
                        string text = GameScreens.FPLAGLKCKII[i].HBJNBEDIFIG.transform.Find("Value").gameObject
                ...
                {
                    OHAJKFJEAFN menu = GameScreens.FPLAGLKCKII[foc];
                    if (dir == 0)
                    {
                        dir = menu.LDPBBENEMHM >= 0 ? 1 : -1;
                    }
                    ...
                    if (dir < 0)
                    {
                        if (foc > 1)
                        {
                            OHAJKFJEAFN menu2 = GameScreens.FPLAGLKCKII[foc - 1];
                            Text text1 = menu.HBJNBEDIFIG.transform.Find("Value").gameObject.GetComponent<Text>();
                            Text text2 = menu2.HBJNBEDIFIG.transform.Find("Value").gameObject.GetComponent<Text>();
            ...
            if (GameScreens.OBNLIIMODBI == 1002)
            {
                ...
                for (int num = 1; num <= GameScreens.CFPJFAKOKMD; num++)
                {
                    if (GameScreens.FPLAGLKCKII[num].LDPBBENEMHM != 0f && GameScreens.KGNNPFLHMPF == 0 &&
                        ...
                        GameScreens.FPLAGLKCKII[num].LFBLCLADKOM() == 0)
                    {
    ...
    public static bool GameScreens_ICKGKDOKJEN(int PBKICCGONGP)
    {
        if (GameScreens.OBNLIIMODBI > 1000)
        {
            ...
            for (GameScreens.BCAKDEKDCHC = 1;
                 GameScreens.BCAKDEKDCHC <= GameScreens.CFPJFAKOKMD;
                 GameScreens.BCAKDEKDCHC++)
            {
